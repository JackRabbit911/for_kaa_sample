...
class Im 
{
	...
	public function __construct($file)
	{
		...
		{
			throw new WnException('Not an image or invalid image: :file',
	...
	public function resize($width = NULL, $height = NULL, $master = NULL)
	{
		...
		elseif ($master == static::WIDTH AND ! empty($width))
		{
			$master = static::AUTO;
		...
		elseif ($master == static::HEIGHT AND ! empty($height))
		{
			$master = static::AUTO;
		...
		switch ($master)
		{
			case static::AUTO:
				...
				$master = ($this->width / $width) > ($this->height / $height) ? static::WIDTH : static::HEIGHT;
			break;
			case static::INVERSE:
				// Choose direction with the minimum reduction ratio
				$master = ($this->width / $width) > ($this->height / $height) ? static::HEIGHT : static::WIDTH;
			break;
		}
	...
	{
		if ($direction !== static::HORIZONTAL)
		{
			// Flip vertically
			$direction = static::VERTICAL;
		}
	 ...
	 */
	public function watermark(Im $watermark, $offset_x = NULL, $offset_y = NULL, $opacity = 100)
	{
		if ($offset_x === NULL)
		{
			// Center the X offset
	...
	public function save($file = NULL, $quality = 100)
	{
		...
		if (is_file($file))
		{
			if ( ! is_writable($file))
			{
				throw new WnException('File must be writable: :file',
					array(':file' => Debug::path($file)));
			}
		}
		else
		{
			...
			{
				throw new WnException('Directory must be writable: :directory',
	 ...
	 * @return  void
	 */
	protected function _do_watermark(Im $image, $offset_x, $offset_y, $opacity){}
