	...
	public static function path($array, $path, $default = NULL, $delimiter = NULL)
	{
		...
		do
		{
			...
			elseif ($key === '*')
			{
				...
				{
					if ($value = Arr::path($arr, $keys)) // implode('.', $keys)))
					{
						$values[] = $value;
					}
				}
...
//        return wn_array_get($array, $key, $default);
	}
	 ...
	 * If values are not unique, create keys array as a new value.
	 * 
	 *  `   $array = ['key1'=>'val1', 'key2'=>'val2', 'key3'=>'val1'];
	 *      $array = Arr::flip($array);
	 *      // The output of $array will now be:
	...
	public static function flip($array)
	{
		...
		foreach($array AS $key=>$value)
		{
			if(!isset($res[$value])) $res[$value] = $key;
			else
			{
	...
	/**
	 * returns an array containing the elements of the array 1, 
	 * the keys that correspond to the values of the array 2
	 * 
	 *  `   $array = ['foo'=>'foo', 'bar1'=>'bar', 'baz'=>'baz'];
	 *      $keys = ['foo', 'bar1'=>'bar'];
	 *      $array = Arr::intersect_keys($array, $keys);
	 *      // The output of $array will now be:
	 *      ['foo'=>'foo', 'bar'=>'bar']
	 * 
	 * @param array $array
	 ...
	 * @author WN Team.
	 */
	public static function intersect_keys(array $array, array $keys)
	{            
		$result = [];
		...
		foreach($keys AS $key=>$value)
		{
			...
			{
				if(isset($array[$key])) $result[$value] = $array[$key];
			}
		}
	...
	{
		if(count($array) - count($array, COUNT_RECURSIVE) !== 0) return TRUE;
		else return FALSE;
	}
	 ...
	 * Example:
	 * $haystack = [
	 *  'woman' => ['girl', 'madam', 'woman', 'babushka'],
	 *  'man' => ['man', 'boy', 'gay', 'mister'],
	 * ];
	 * if $needle == 'boy', return 'man',
	 * if $needle == 'bull' return 'bull'
	 * 
	 * @param array $array
	 * @return boolean
	 * @author WN Team.
	 */
	public static function get_key($array, $value)
	{
		foreach($array AS $key => $values)
		{
			if(is_array($values) && in_array($value, $values)) return $key;
			elseif(!is_array($values) && $values == $value) return $key;
		}
	...
	/**        
	 * Removes duplicate values(arrays) from a multidimensional array
	 * 
	 * @param array $array
	 * @return array
	 * @author WN Team.
 ...
 * @author WN Team.
 */
	public static function replace_values(array $array, array $replace)
	{
		foreach($array AS $key=>$value)
		{
			if(is_array($value)) $array[$key] = static::replace_values($value, $replace);
			elseif(isset($replace[$value])) $array[$key] = $replace[$value];
		}
	...
	public static function unset_path(& $array, $path, $delimiter = NULL)
	{
		...
		do
		{
			...
			if (isset($array[$key]))
			{
				...
				{
					// Found the path requested, unset and return true
					unset($array[$key]);
					return TRUE;
				}
			}
	...
	public static function flush(& $array, $key, $default = null)
	{
	...
	public static function insert($array, $replacement, $position)
	{
		return array_slice($array, 0, $position, true) + $replacement +
