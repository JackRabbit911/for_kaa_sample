...
class File 
{
	...
	public static function mime_by_ext($extension)
	{
		...
		return Config::instance()->get('mimes', $extension, Config::SETTINGS);
	}
	...
	public static function mimes_by_ext($extension)
	{
		// Load all of the mime types
		// $mimes = Core::config('mimes');
		$mimes = Config::instance()->get('mimes');
	...
	public static function exts_by_mime($type)
	{
		...
		if (empty($types))
		{
			foreach (Config::instance()->get('mimes') as $ext => $mimes)
			{
	...
	public static function type_by_filename($filename)
	{
		$mime = static::mime($filename);
		return static::type_by_mime($mime);
	}

	public static function type_by_mime($mime)
	{
		return explode('/', $mime)[0];
	}

	public static function relative_path($filename)
	{
